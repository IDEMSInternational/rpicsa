...
start_rains <- function(data, date_time, station = NULL, year = NULL, rain = NULL, threshold = 0.85,
  ...
  checkmate::assert_logical(total_rainfall)
  checkmate::assert_logical(proportion)
  ...
  checkmate::assert_int(over_days, lower = 1)
  ...
  if ((dry_period) & (period_interval < period_max_dry_days)) stop("Value given in `period_interval` must be equal to or greater than the value given in `period_max_dry_days`")

...
#' @param s_start_month \code{numerical(1)} Default `NULL` (if `NULL`, `s_start_month = 1`. The month to state is the first month of the year.
...
start_rains <- function(data, date_time, station = NULL, year = NULL, rain = NULL, threshold = 0.85,
                        doy = NULL, start_day = 1, end_day = 366, s_start_month = NULL,
                        drop = TRUE,
                        ...
                        evaporation = FALSE, evaporation_variable = NULL, fraction = 0.5, data_book = NULL) {
  ...
  if (evaporation) {
    ...
    roll_sum_evap <- instatCalculations::instat_calculation$new(
      type="calculation", 
      function_exp=paste0("RcppRoll::roll_sumr(x=fraction_evap, n=", over_days, ", fill=NA, na.rm=FALSE)"), 
  ...
  if (station){
    roll_sum_rain <- instatCalculations::instat_calculation$new(
      type="calculation", 
      function_exp=paste0("RcppRoll::roll_sumr(x=", rain, ", n=", over_days, ", fill=NA, na.rm=FALSE)"), 
  ...
  if (number_rain_days && dry_spell && dry_period){
    ...
    if (total_rainfall && proportion) {
      conditions_filter <- instatCalculations::instat_calculation$new(
        type="filter", 
        function_exp=paste0("((", rain, " >= ", threshold, ") & roll_sum_rain > wet_spell & roll_n_rain_days >= ", min_rain_days, " & roll_max_dry_spell <= ", spell_max_dry_days, " & n_dry_period == 0) | is.na(x=", rain, ") | is.na(x=roll_sum_rain) | is.na(x=roll_n_rain_days) | is.na(x=roll_max_dry_spell) | is.na(x=n_dry_period)"), 
        sub_calculations=list(roll_sum_rain, roll_n_rain_days, roll_max_dry_spell, n_dry_period, total_rainfall_wet_spell))
    }
    else if (evaporation){
      conditions_filter <- instatCalculations::instat_calculation$new(
        type="filter", 
        function_exp=paste0("((", rain, " >= ", threshold, ") & roll_sum_rain > roll_sum_evap & roll_n_rain_days >= ", min_rain_days, " & roll_max_dry_spell <= ", spell_max_dry_days, " & n_dry_period == 0) | is.na(x=", rain, ") | is.na(x=roll_sum_rain) | is.na(x=roll_n_rain_days) | is.na(x=roll_max_dry_spell) | is.na(x=n_dry_period)"), 
        sub_calculations=list(roll_sum_rain, roll_sum_evap, roll_n_rain_days, roll_max_dry_spell, n_dry_period))
    }
    else {
      conditions_filter <- instatCalculations::instat_calculation$new(
        type="filter", 
        function_exp=paste0("((", rain, " >= ", threshold, ") & roll_sum_rain > ", amount_rain, " & roll_n_rain_days >= ", min_rain_days, " & roll_max_dry_spell <= ", spell_max_dry_days, " & n_dry_period == 0) | is.na(x=", rain, ") | is.na(x=roll_sum_rain) | is.na(x=roll_n_rain_days) | is.na(x=roll_max_dry_spell) | is.na(x=n_dry_period)"), 
        sub_calculations=list(roll_sum_rain, roll_n_rain_days, roll_max_dry_spell, n_dry_period))
    }
    ...
    if ("doy" %in% output){
      start_of_rains_doy <- instatCalculations::instat_calculation$new(
        type="summary", 
        function_exp=paste0("ifelse(test=is.na(x=dplyr::first(x=", rain, ")) | is.na(x=dplyr::first(x=roll_sum_rain)) | is.na(x=dplyr::first(x=roll_n_rain_days)) | is.na(x=dplyr::first(x=roll_max_dry_spell)) | is.na(x=dplyr::first(x=n_dry_period)), yes=NA, no=dplyr::first(x=", doy, ", default=NA))"), 
    ...
    if ("date" %in% output){
      start_rain_date <- instatCalculations::instat_calculation$new(
        type="summary", 
        function_exp=paste0("dplyr::if_else(condition=is.na(x=dplyr::first(x=", rain, ")) | is.na(x=dplyr::first(x=roll_sum_rain)) | is.na(x=dplyr::first(x=roll_n_rain_days)) | is.na(x=dplyr::first(x=roll_max_dry_spell)) | is.na(x=dplyr::first(x=n_dry_period)), true=as.Date(NA), false=dplyr::first(", date, ", default=NA))"), 
  ...
  if (number_rain_days){
    ...
    if (total_rainfall && proportion){
      conditions_filter <- instatCalculations::instat_calculation$new(
        type="filter", 
        function_exp=paste0("((", rain, " >= ", threshold, ") & roll_sum_rain > wet_spell & roll_n_rain_days >= ", min_rain_days, ") | is.na(x=", rain, ") | is.na(x=roll_sum_rain) | is.na(x=roll_n_rain_days)"), 
        sub_calculations=list(roll_sum_rain, roll_n_rain_days, total_rainfall_wet_spell))
    }
    else if (evaporation){
      conditions_filter <- instatCalculations::instat_calculation$new(
        type="filter", 
        function_exp=paste0("((", rain, " >= ", threshold, ") & roll_sum_rain > roll_sum_evap & roll_n_rain_days >= ", min_rain_days, ") | is.na(x=", rain, ") | is.na(x=roll_sum_rain) | is.na(x=roll_n_rain_days)"), 
        sub_calculations=list(roll_sum_rain, roll_sum_evap, roll_n_rain_days))
    }
    else {
      conditions_filter <- instatCalculations::instat_calculation$new(
        type="filter", 
        function_exp=paste0("((", rain, " >= ", threshold, ") & roll_sum_rain > 20 & roll_n_rain_days >= ", min_rain_days, ") | is.na(x=", rain, ") | is.na(x=roll_sum_rain) | is.na(x=roll_n_rain_days)"), 
        sub_calculations=list(roll_sum_rain, roll_n_rain_days))
    }
    ...
    if ("doy" %in% output){
      start_of_rains_doy <- instatCalculations::instat_calculation$new(
        type="summary", 
        function_exp=paste0("ifelse(test=is.na(x=dplyr::first(x=", rain, ")) | is.na(x=dplyr::first(x=roll_sum_rain)) | is.na(x=dplyr::first(x=roll_n_rain_days)), yes=NA, no=dplyr::first(x=", doy, ", default=NA))"), 
        result_name="start_rain", 
        save=2)
    }
    if ("date" %in% output){
      start_rain_date <- instatCalculations::instat_calculation$new(
        type="summary", 
        function_exp=paste0("dplyr::if_else(condition=is.na(x=dplyr::first(x=", rain, ")) | is.na(x=dplyr::first(x=roll_sum_rain)) | is.na(x=dplyr::first(x=roll_n_rain_days)), true=as.Date(NA), false=dplyr::first(", date, ", default=NA))"), 
        result_name="start_rain_date", 
        save=2)
    }
  }
  if (dry_spell){
    ...
    if (total_rainfall && proportion){
      conditions_filter <- instatCalculations::instat_calculation$new(
        type="filter", 
        function_exp=paste0("((", rain, " >= ", threshold, ") & roll_sum_rain > wet_spell & roll_max_dry_spell <= ", spell_max_dry_days, ") | is.na(x=", rain, ") | is.na(x=roll_sum_rain) | is.na(x=roll_max_dry_spell)"), 
        sub_calculations=list(roll_sum_rain, roll_max_dry_spell, total_rainfall_wet_spell))
    }
    else if (evaporation) {
      conditions_filter <- instatCalculations::instat_calculation$new(
        type="filter", 
        function_exp=paste0("((", rain, " >= ", threshold, ") & roll_sum_rain > roll_sum_evap & roll_max_dry_spell <= ", spell_max_dry_days, ") | is.na(x=", rain, ") | is.na(x=roll_sum_rain) | is.na(x=roll_max_dry_spell)"), 
        sub_calculations=list(roll_sum_rain, roll_sum_evap, roll_max_dry_spell))
    }
    else {
      conditions_filter <- instatCalculations::instat_calculation$new(
        type="filter", 
        function_exp=paste0("((", rain, " >= ", threshold, ") & roll_sum_rain > ", amount_rain, " & roll_max_dry_spell <= ", spell_max_dry_days, ") | is.na(x=", rain, ") | is.na(x=roll_sum_rain) | is.na(x=roll_max_dry_spell)"), 
        sub_calculations=list(roll_sum_rain, roll_max_dry_spell))
    }
    ...
    if ("doy" %in% output){
      start_of_rains_doy <- instatCalculations::instat_calculation$new(
        type="summary", 
        function_exp=paste0("ifelse(test=is.na(x=dplyr::first(x=", rain, ")) | is.na(x=dplyr::first(x=roll_sum_rain)) | is.na(x=dplyr::first(x=roll_max_dry_spell)), yes=NA, no=dplyr::first(x=", doy, ", default=NA))"), 
        result_name="start_rain", 
        save=2)
    }
    if ("date" %in% output){
      start_rain_date <- instatCalculations::instat_calculation$new(
        type="summary", 
        function_exp=paste0("dplyr::if_else(condition=is.na(x=dplyr::first(x=", rain, ")) | is.na(x=dplyr::first(x=roll_sum_rain)) | is.na(x=dplyr::first(x=roll_max_dry_spell)), true=as.Date(NA), false=dplyr::first(", date, ", default=NA))"), 
        result_name="start_rain_date", 
        save=2)
    }
  ...
  if (dry_period){
    ...
    if (total_rainfall && proportion){
      conditions_filter <- instatCalculations::instat_calculation$new(
        type="filter", 
        function_exp=paste0("((", rain, " >= ", threshold, ") & roll_sum_rain > wet_spell & n_dry_period == 0) | is.na(x=", rain, ") | is.na(x=roll_sum_rain) | is.na(x=n_dry_period)"), 
        sub_calculations=list(roll_sum_rain, n_dry_period, total_rainfall_wet_spell))
    }
    else if (evaporation){
      conditions_filter <- instatCalculations::instat_calculation$new(
        type="filter", 
        function_exp=paste0("((", rain, " >= ", threshold, ") & roll_sum_rain > roll_sum_evap & n_dry_period == 0) | is.na(x=", rain, ") | is.na(x=roll_sum_rain) | is.na(x=n_dry_period)"), 
        sub_calculations=list(roll_sum_rain, roll_sum_evap, n_dry_period))
    }
    else {
      conditions_filter <- instatCalculations::instat_calculation$new(
        type="filter", 
        function_exp=paste0("((", rain, " >= ", threshold, ") & roll_sum_rain > ", amount_rain, " & n_dry_period == 0) | is.na(x=", rain, ") | is.na(x=roll_sum_rain) | is.na(x=n_dry_period)"), 
        sub_calculations=list(roll_sum_rain, n_dry_period))
    }
    ...
    if ("doy" %in% output) {
      start_of_rains_doy <- instatCalculations::instat_calculation$new(
        type="summary", 
        function_exp=paste0("ifelse(test=is.na(x=dplyr::first(x=", rain, ")) | is.na(x=dplyr::first(x=roll_sum_rain)) | is.na(x=dplyr::first(x=n_dry_period)), yes=NA, no=dplyr::first(x=", doy, ", default=NA))"), 
        result_name="start_rain", 
        save=2)
    }
    if ("date" %in% output){
      start_rain_date <- instatCalculations::instat_calculation$new(
        type="summary", 
        function_exp=paste0("dplyr::if_else(condition=is.na(x=dplyr::first(x=", rain, ")) | is.na(x=dplyr::first(x=roll_sum_rain)) | is.na(x=dplyr::first(x=n_dry_period)), true=as.Date(NA), false=dplyr::first(", date, ", default=NA))"), 
        result_name="start_rain_date", 
        save=2)
    }        
  }
  ...
  if (!number_rain_days && !dry_spell && !dry_period) {
    if (total_rainfall && proportion){
      conditions_filter <- instatCalculations::instat_calculation$new(
        type="filter", 
        function_exp=paste0("((", rain, " >= ", threshold, ") & roll_sum_rain > wet_spell) | is.na(x=", rain, ") | is.na(x=roll_sum_rain)"), 
        sub_calculations=list(roll_sum_rain, total_rainfall_wet_spell))
    }
    else if (evaporation) {
      conditions_filter <- instatCalculations::instat_calculation$new(
        type="filter", function_exp=paste0("((", rain, " >= ", threshold, ") & roll_sum_rain > roll_sum_evap) | is.na(x=", rain, ") | is.na(x=roll_sum_rain)"), 
        sub_calculations=list(roll_sum_rain, roll_sum_evap))
    }
    else {
      conditions_filter <- instatCalculations::instat_calculation$new(
        type="filter", 
        function_exp=paste0("((", rain, " >= ", threshold, ") & roll_sum_rain > ", amount_rain, ") | is.na(x=", rain, ") | is.na(x=roll_sum_rain)"), 
        sub_calculations=list(roll_sum_rain))
    }
  }
  ...
  doy_filter <- instatCalculations::instat_calculation$new(
    type="filter", 
    function_exp=paste0(doy, " >= 1 & ", doy, " <= 366"), 
  ...
  if (!number_rain_days && !dry_spell && !dry_period){
    if ("doy" %in% output){
      start_of_rains_doy <- instatCalculations::instat_calculation$new(
        type="summary", 
        function_exp=paste0("ifelse(test=is.na(x=dplyr::first(x=", rain, ")) | is.na(x=dplyr::first(x=roll_sum_rain)), yes=NA, no=dplyr::first(x=", doy, ", default=NA))"), 
        result_name="start_rain", 
        save=2)
    }
    if ("data" %in% output){
      start_rain_date <- instatCalculations::instat_calculation$new(
        type="summary", 
        function_exp=paste0("dplyr::if_else(condition=is.na(x=dplyr::first(x=", rain, ")) | is.na(x=dplyr::first(x=roll_sum_rain)), true=as.Date(NA), false=dplyr::first(", date, ", default=NA))"), 
        result_name="start_rain_date", 
        save=2)
    }
  }
